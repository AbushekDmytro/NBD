...
object Assignment_8_Abushek Dmytro extends App {
  ...
  case class PersonLite(var firstName: String, var lastName: String) {
    def greeting(person: PersonLite): Unit = person match {
      case PersonLite("Dmytro", _) => println("Hello Dmytro")
      case PersonLite(_, "Abushek") => println("Good Afternoon Mr.Dmytro")
      case PersonLite(firstN, lastN) => println(s"Nice to meet you $firstN $lastN")
    }
  }
  val Person1: PersonLite = PersonLite("Donald", "Trump")
  Person1.greeting(PersonLite("Dmytro", "Surname"))
  Person1.greeting(PersonLite("Name", "Abushek"))
  ...
  var student = new Person("Dima", "Abushek", 0) with Student
  println("Student")
