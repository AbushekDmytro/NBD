...
object Assignment_7_AbushekDmytroApp extends App {
  println("Hello World")
...
//        print(newI)
  ...
  def AllDaysWithRecursionFromLast(strList: List[String]): String = {
    if (strList.isEmpty) ""
    else strList.last + ", " + AllDaysWithRecursionFromLast(strList.splitAt(strList.length-1)._1) 
  }
